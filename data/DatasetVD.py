...
class DatasetVD(torch.utils.data.Dataset):
    def __init__(self,cfg,is_train=True):
        ...
        if is_train : 
            self.list_noisy = get_list(cfg.data.train.noisy,"*.wav")
        else :
            self.list_noisy = glob(join(cfg.data.valid.root,"*","noisy","*.wav"),recursive=True)

        self.len_data = cfg.data.get("len_data",48000)
        self.sr = cfg.data.sr
        ...
        if is_train : 
            ...
            print("DatasetVD[valid:{}] : {}".format(is_train,len(self.list_noisy)))
    ...
    def get_clean_path(self,path_noisy):
        ...
        if self.is_train : 
            ...
            path_clean = os.path.join(self.cfg.data.train.clean,base_name)
        else : 
            path_after_root = path_noisy.split(self.cfg.data.valid.root)[-1]
            dev_type = path_after_root.split("/")[0]

            fid = path_noisy.split("_")[-1]
            fid = fid.split(".")[0]

            path_clean = os.path.join(self.cfg.data.valid.root,dev_type,"clean","clean_fileid_{}.wav".format(fid))
...
if __name__ == "__main__" : 
    import sys
    sys.path.append("../")
    from utils.cfgarams import cfgaram
    cfg = cfgaram("../../config/mpSEv2/v138.yaml","../../config/mpSEv2/default.yaml")

    db = DatasetVD(cfg,is_train=True)
