...
class TF_CorrNet_helper(nn.Module):
    ...
    # #### multi channel
    # def forward(self, x):
    #     mixture_stft = self.stft(x.permute(0,2,1), cplx=True)
    #     mixture_stft = mixture_stft.permute(0, 3, 2, 1).contiguous()
    #     x_input = torch.cat([torch.real(mixture_stft), torch.imag(mixture_stft)],dim=-1)
    #     mixture_stft = mixture_stft.permute(0, 3, 2, 1).contiguous()
...
if __name__ == "__main__":
    ...
    device = "cuda:0"
    model = TF_CorrNet_helper(hp).to(device)
    model.eval()
    
    model.load_state_dict(torch.load('log/chkpt/whisper_mwMSE_frame_snr_06/bestmodel1.pt', weights_only=True, map_location="cuda:0"))

    input_dir = "mpANC/test"
    output_dir = "mpANC/test_after"
    # input_dir = "/mnt/hdisk1/LibriTest"
    # output_dir = "/mnt/hdisk1/LibriTest_small_16151"
    ...
    for wav in wavList:
        ...
        for i in range(0, len(x_tensor), stride):
            ...
            with torch.no_grad():
                y_chunk = model(chunk).squeeze(0).cpu()[:len(x)]
            output_chunks.append(y_chunk)

        y = torch.cat(output_chunks, dim=-1).numpy()
