...
from .bottleneck import RNNFormerBlock, RNNFormerConfig, rf_pre_post_lin, rf_pre_post_erb

class TinyHencer(nn.Module):
    def __init__(
        self,
        n_fft = 512,
        channels: int = 64,
        ...
        **kwargs
        ...
        if domain == "none" : 
            print(f"TinyHencer: Using default pre/post encoder/decoder.")
            ...
            print(f"TinyHencer: Using mag pre/post encoder/decoder.")
        ...
        # RNNFormer PreNet
        freq = n_fft // 2 // stride
        if "linear" in pre_post_init:
            rf_pre, rf_post = rf_pre_post_lin(freq, self.rf_freq, pre_post_init, bias=False)
        elif "erb" in pre_post_init:
            rf_pre, rf_post = rf_pre_post_erb(freq, self.rf_freq, pre_post_init, bias=False)
        else:
            raise RuntimeError(f"model_kwargs.pre_post_init={pre_post_init} is not supported.")
    ...
    def to_onnx(self, output_fp, device=torch.device("cpu")):
        print("ONNX export is not implemented yet for TinyHencer.")

        dummy_spec = torch.randn(1, 256, 1, 2).to(device)
        dummy_hs = [torch.randn(1, self.rf_freq, self.rf_ch).to(device) for _ in range(len(self.rf_block))]
        ...
        torch.onnx.export(
            ...
            verbose=False,
            opset_version=16,
            input_names=["spec_noisy", "hs_in"],
            output_names=["spec_estim", "hs_out"],
            dynamo=False
        )

class TinyHencerWrapper(nn.Module):
    def __init__(self,
            n_fft: int = 512,
            hop_size : int = 256,
        ...
        self.model = TinyHencer(n_fft=n_fft,**kwargs)
